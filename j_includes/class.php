	...
	class Planter {
		...
		function register($redirect) {
			...
			if ( !empty ( $_POST ) ) {
				...
				if ( $referrer == $current ) {
					...
					$userpass = $pdb->hash_password($userpass, $nonce);
					...
					$insert = $pdb->insert($link, $table, $fields, $values);
		...
		function login($redirect) {
			...
			if ( !empty ( $_POST ) ) {
				...
				$subpass = $pdb->hash_password($subpass, $nonce);
				...
				if ( $subpass == $stopass ) {
					...
					$authID = $pdb->hash_password($subpass, $authnonce);
					
					//Set our authorization cookie
					setcookie('planterlogauth[user]', $subname, 0, '', '', '', true);
					setcookie('planterlogauth[authID]', $authID, 0, '', '', '', true);
		...
		function logout() {
			...
			$idout = setcookie('planterlogauth[authID]', '', -3600, '', '', '', true);
			$userout = setcookie('planterlogauth[user]', '', -3600, '', '', '', true);
		...
		function checkLogin() {
			...
			if ( !empty ( $cookie ) ) {
				...
				$stopass = $pdb->hash_password($stopass, $authnonce);
